...
export default function Settings() {
  ...
  const toggleListSeparator = (state) => {
    setSeparateList((prevState) => ({ ...prevState, [state]: !prevState[state] }));
  };
      ...
      <Collapse in={serverType === 'restricted'} timeout="auto" sx={{ overflow: 'auto' }}>
        <CardContent>
          <CardActions sx={{ paddingX: 0 }}>
            <FormControlLabel
              control={<Checkbox checked={separateList.current} onChange={(e) => toggleListSeparator('current', e)} />}
