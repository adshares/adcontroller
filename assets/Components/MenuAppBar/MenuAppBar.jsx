    ...
    <AppBar position="sticky" color="muted" sx={{ boxShadow: 'none' }}>
      <Toolbar>
        ...
        {showProtectedOptions && mode === 'app' && (
          <IconButton size="large" color="secondaryAlt" onClick={() => toggleSideMenu(!showSideMenu)}>
            <MenuIcon />
          </IconButton>
        )}
        {showProtectedOptions && (
          <Box sx={{ ml: 'auto' }} className={`${commonStyles.flex} ${commonStyles.alignCenter}`}>
            {appData.AdPanel.Url && (
              <Link href={appData.AdPanel.Url} color="secondaryAlt" underline="hover" variant="body1">
                BACK TO ADPANEL
              </Link>
            )}
            <IconButton size="large" onClick={handleMenu} color="secondaryAlt">
